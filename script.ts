 ...
 */
const drawNought = function(row: number, column: number) {
 ...
 */
const previewNought = function(row: number, column: number) {
    if (row < 0 || row > 2 || column < 0 || column > 2) throw new RangeError("Invalid row or column index");
 ...
 */
const drawCross = function(row: number, column: number) {
    if (row < 0 || row > 2 || column < 0 || column > 2) throw new RangeError("Invalid row or column index");
 ...
 */
const previewCross = function(row: number, column: number) {
    if (row < 0 || row > 2 || column < 0 || column > 2) throw new RangeError("Invalid row or column index");
...
class Grid {
     ...
     */
    checkRow() {
        const token: string = this.cells[lastRow][lastColumn].token!;
